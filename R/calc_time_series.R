...
calc_time_series <- function(dat_cleaned, dat_prepped, multinomial_model, gam_posterior) {
  ...
  interps <- dat_prepped |>
    ...
    group_by(day) |>
    summarize(
      frac_subadult = unique(frac_subadult),
  ...
  newdat <- crossing(
    ...
    # fmi_kg_m2 = multinomial_model$model$fmi_kg_m2 |> table() |> sort() |> rev() |> names() |> head(1) |> as.numeric(), # nolint
    # frac_subadult = multinomial_model$model$frac_subadult |> table() |> sort() |> rev() |> names() |> head(1) |> as.numeric(), # nolint
