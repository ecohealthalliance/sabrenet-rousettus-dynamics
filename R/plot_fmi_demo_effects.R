...
plot_fmi_demo_effects <- function(dat_prepped) {
  ...
  fig_fmi_demo_effects <- ggplot(point_plot_data, aes(fill = demo_group, label = demo_group, x = mean_fmi, y = mean, ymin = lower, ymax = upper, xmin = mean_fmi - 2*sd_fmi, xmax = mean_fmi + 2*sd_fmi, color = demo_group)) +
...
plot_fmi_demo_timeseries <- function(dat_prepped) {
  ...
  fig_fmi_demo_timeseries <- ggplot(plot_data, aes(x = date, y = mean_fmi, ymin = mean_fmi - 2*sd_fmi, ymax = mean_fmi + 2*sd_fmi, color = demo_group, fill  = demo_group)) +
...
plot_positivity_demo_timeseries <- function(dat_prepped) {
  plot_data <- dat_prepped |>
    filter(sample_type == "Rectal") |>
    group_by(date, demo_group) |>
